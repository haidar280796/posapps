...
import { type BreadcrumbItem, type StockAdjustment } from '@/types';
...
const breadcrumbs: BreadcrumbItem[] = [
    {
        ...
        href: route('trn.stock_adjustments.index'),
    },
];
...
// Fungsi pencarian
const search = () => {
    formSearch.get(route("trn.stock_adjustments.index"), {
...
const deleteData = (e: Event) => {
    ...
    formDelete.delete(route('trn.stock_adjustments.destroy', `${confirmingDeletionid.value}`), {
        preserveScroll: true,
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="px-4 py-6">
            <Heading title="Stocks Adjustments" description="Manage your data stocks adjustments" />


            <div class="flex items-center gap-4 mb-6">
                <Link :href="route('trn.stock_adjustments.create')"
            ...
            <div class="overflow-x-auto">
                <table
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        <template v-if="stockAdjustments.data.length === 0">
                            <tr>
                                <td colspan="4"
                        ...
                        <template v-else>
                            <tr v-for="stockAdjustment in stockAdjustments.data" v-bind:key="stockAdjustment.id">
                                <td class="px-4 py-2 font-medium whitespace-nowrap text-gray-900 dark:text-white">
                                    {{ stockAdjustment.warehouse?.nama_gudang }}
                                </td>
                                <td class="px-4 py-2 whitespace-nowrap text-gray-700 dark:text-gray-200">{{
                                    stockAdjustment.product?.nama_produk }}</td>
                                <td class="px-4 py-2 whitespace-nowrap text-gray-700 dark:text-gray-200">{{
                                   `${formatRibu(stockAdjustment.jumlah)} ${stockAdjustment.product?.satuan_dasar?.nama_satuan}`  }}</td>
                                <td class="px-4 py-2 whitespace-nowrap w-20">
                                    <span
                                        ...
                                        <button @click.prevent="() => confirmDeletion(stockAdjustment.id)" method="delete"
            ...
            <div class="py-2 mt-4">
                <ol class="flex justify-end gap-1 text-xs font-medium">
                    <template v-for="(link, index) in stockAdjustments.links" :key="index">
                        <li>
