...
// Fungsi pencarian
const search = () => {
    formSearch.get(route("md.products.index"), {
...
const deleteData = (e: Event) => {
    ...
    formDelete.delete(route('md.products.destroy', `${confirmingDeletionid.value}`), {
        preserveScroll: true,
...
const cloneData = (id: string | number) => {
    const form = useForm({});

    form.patch(route('md.products.clone', id), {
        preserveScroll: true,
        onSuccess: () => closeModal(),
        onFinish: () => form.reset(),
    });
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="px-4 py-6">
            ...
            <div class="flex items-center mb-6">
                <Link :href="route('md.products.create')"
                            ...
                            <tr v-for="product in products.data" v-bind:key="product.id">
                                ...
                                <td class="px-4 py-2 whitespace-nowrap w-20">
                                    <span
                                        ...
                                        <Link :href="route('md.products.edit', product.id)"
                                        ...
                                        <button @click.prevent="() => confirmDeletion(product.id)" method="delete"
                                        ...
                                        <button @click.prevent="() => cloneData(product.id)" method="post"
                                            class="inline-block p-3 text-gray-700 hover:bg-gray-50 focus:relative dark:text-gray-200 dark:hover:bg-blue-600"
                                            title="Clone Data">
                                            <Copy class="size-4" />
                                        </button>
